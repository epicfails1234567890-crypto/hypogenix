...
func generar_mundo():
	...
	# 1. Habilitamos colores de vértice para no tener que duplicar el material
	# Esto permite que CADA bloque tenga su color sin crear miles de materiales
	material_comun.vertex_color_use_as_albedo = true 
	
	# 2. Triplanar corregido
	material_comun.uv1_triplanar = true
	material_comun.uv1_world_triplanar = true
	material_comun.uv1_scale = Vector3(1, 1, 1)
	
	# 3. Ajuste de Iluminación (Vital para Intel HD / GPUs integradas)
	material_comun.diffuse_mode = BaseMaterial3D.DIFFUSE_LAMBERT
	material_comun.specular_mode = BaseMaterial3D.SPECULAR_DISABLED # Evita brillos negros
	material_comun.roughness = 1.0 # Material totalmente mate (tierra)
	
	for x in range(tamaño):
		for z in range(tamaño):
			var altura_max = int(round(noise.get_noise_2d(x, z) * 100) + 100)
...
func obtener_color_por_altura(y, surface):
	...
	if y < 125: color_bloque = Color("fff000") # Amarillo
	return color_bloque
